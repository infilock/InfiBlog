...
func NewSocialMediaRepository(db *sql.DB) Repository {
	return &socialMediaRepo{db: db}
}

func (repo *socialMediaRepo) FindTweet(ctx context.Context, questionID string) (*Entity, error) {
	var m Entity
...
func (repo *socialMediaRepo) ListSocialByStatus(ctx context.Context, social, status string) ([]Entity, error) {
...
func (repo *socialMediaRepo) Create(ctx context.Context, m []Entity) error {
	for i := 0; i < len(m); i++ {
		args := []interface{}{
			m[i].QuestionID,
			m[i].Type,
