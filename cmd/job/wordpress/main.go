...
func main() {
	location, err := time.LoadLocation(config.WordPressJobLocation.Get())
	if err != nil {
		panic(err)
	}
	...
	_, errJobOne := scheduler.Every(1).Day().At(config.WordPressJobTime.Get()).Do(func() {
		...
		qr := pool.NewQuestionRepository(database)
		items, errList := articleRepo.ListStatus(context.Background(), "draft")
		if errList != nil {
			log.Fatal(errList)
		}
		...
		for i := 0; i < len(items); i++ {
			...
			//_, errStat := os.Stat(config.UploadPath.Get())
			//if errStat != nil {
			...
			req, errRequest := http.NewRequestWithContext(ctx, http.MethodPost, config.WordPressAPICreateArticle.Get(), payload)
			if errRequest != nil {
				log.Fatal("error on new request with context: ", errRequest)
			}

			req.SetBasicAuth(config.WordPressUsername.Get(), config.WordPressPassword.Get())
...
//	defer cancel()
//
//	req, errNewRequest := http.NewRequestWithContext(ctx, http.MethodPost, config.WordPressAPICreateMedia.Get(), payload)
...
//			}
//
//			fileDir, errGrab := grab.Get(config.UploadPath.Get(), gs.Items[0].Link)
//			if errGrab != nil {
